    ...
    <div class="results">
        <ClientOnly>
            <div class="warning" v-if="isLowPercent">
                ...
                <p >
                    <i>{{ getUserName }}</i>, похоже, вы очень уникальная личность. Наша нейронная сеть сомневается в твоём случае, однако мы всё равно сделаем предположение.
                </p>
                <p>
                    Группа определялась на основании ваших подписок. Чтобы попытаться определить направление, предлагаем пройти тест.
                </p>
            </div>
            <div class="warning" v-else>
                ...
                <p>
                    <i>{{ getUserName }}</i>, очередь за вами. Если хочется получить конкретное направление, предлагаем пройти тест.
                </p>
                <p>
                    Группа определялась на основании ваших подписок.
                </p>
            </div>
            <div class="results__list">
                <div class="wrapper" v-for="(group, index) in getGroups">
                    <div class="wrapper__title">
                        <span class="">{{ Intl.NumberFormat("ru-RU").format(getRealProbability(index)) }}%</span>
                        <h2>{{ group.group }}</h2>
                    </div>
